...
// For testing the looping functions
...
public class AudioManager {
	...
	public void addSound(String name, String filepath) {
		...
		if (audiolist.containsKey(name)) {
			if(debug){System.out.println("AudioManager: '" + name + "' already exist");}
			
		// If not, create a new instance	
		}else {
			AudioTrack new_sound = new AudioTrack(filepath, (float) 1, false, true);
			audiolist.put(name, new_sound);
			if(debug){System.out.println("AudioManager: Sound '" + name + "' added");}
		}
	}
	...
	public void addMusic(String name, String filepath) {
		...
		// If not, create a new instance	
			...
			audiolist.put(name, new_sound);
			if(debug){System.out.println("AudioManager: Music '" + name + "' added");}
		}
	}
	...
	public void removeAudio(String name) {
		...
		if (!audiolist.containsKey(name)) {
			if(debug){System.out.println("AudioManager: '" + name + "' does not exist");}
		...
		// If exsists, remove it
			...
			if(debug){System.out.println("AudioManager: '" + name + "' removed");}
		}
	}
	...
	public void updatePath(String name, String new_filepath) {
		...
		// If exsists, update the filepath
			...
			if(debug){System.out.println("AudioManager: filepath for '" + name + "' has been updated to '" + new_filepath + "'");}
		}
	}
	...
	public void setLoop(String name, boolean isLoop) {
		...
		// If exsists, update the value of isLoop
			...
			if(debug){System.out.println("AudioManager: '" + name + "' Loop set to " + isLoop);}
		}
	}
	...
	public void setVolume(String name, float new_volume) {
		...
		// If exsists, update the value of isLoop
			...
			if(debug){System.out.println("AudioManager: '" + name + "' Volume set to " + new_volume);}
		}
	}
	...
	public synchronized void setPosition(String name, float playback_position) {
		...
		if (!audiolist.containsKey(name)) {
			...
			if(!audiotrack.isFX) {
				...
				if(debug){System.out.println("AudioManager: '" + name + "' is not Music, cannot set position");};
			}
		}
	}
